...
public class ProjectService {
  ...
  public List<ProjectData> listProjectsForCurrentUser() {
    ...
    // owned projects
    List<ProjectData> result = new ArrayList<>(projectRepository.findAllByEmail(email));
    ...
    if (teamRows != null && !teamRows.isEmpty()) {
      List<Long> teammateProjectIds =
          teamRows.stream().map(ProjectTeam::getProjectId).distinct().collect(Collectors.toList());

      List<ProjectData> teammateProjects = projectRepository.findAllById(teammateProjectIds);
      for (ProjectData p : teammateProjects) {
        boolean exists =
            result.stream().anyMatch(r -> r.getId() != null && r.getId().equals(p.getId()));
